...
class DataIngestion:
    ...
    def ingest(self):
        ...
        all_files = [os.path.join(raw_dir, f)
                     for f in os.listdir(raw_dir) if f.endswith(".csv")]
        df_list = [pd.read_csv(f, parse_dates=True, index_col=0)
                   for f in all_files]
        data = pd.concat(df_list).sort_index()

        train, test = train_test_split(data, shuffle=False, test_size=0.2)
        os.makedirs(self.config.ingested_dir, exist_ok=True)
        train.to_csv(os.path.join(self.config.ingested_dir,
                                  self.config.train_file))
        test.to_csv(os.path.join(self.config.ingested_dir,
                                 self.config.test_file))
        return (
          os.path.join(self.config.ingested_dir, self.config.train_file),
